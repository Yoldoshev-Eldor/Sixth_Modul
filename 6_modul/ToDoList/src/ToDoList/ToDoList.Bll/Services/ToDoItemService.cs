...
namespace ToDoList.Bll.Services
{
    public class ToDoItemService : IToDoItemService
    {
        ...
        public async Task<long> AddToDoItemAsync(ToDoItemCreateDto toDoItem)
        {
            ...
            covert.UserId = long.Parse(userId);
        ...
        {
            var item = await _toDoItemRepository.SelectToDoItemByIdAsync(id, userId);
            if (item is null)
            {
        ...
        public async Task<ToDoItemGetDto> GetToDoItemByIdAsync(long id, long userId)
        {
            var founded = await _toDoItemRepository.SelectToDoItemByIdAsync(id, userId);
            if (founded == null)
            {
                throw new NotFoundException($"ToDoItem with id {id} not found.");
            }
        ...
        public async Task UpdateToDoItemAsync(ToDoItemUpdateDto newItem, long userId)
        {
            ...
            await _toDoItemRepository.UpdateToDoItemAsync(_mapper.Map<ToDoItem>(newItem),userId);
        }
    }
}
