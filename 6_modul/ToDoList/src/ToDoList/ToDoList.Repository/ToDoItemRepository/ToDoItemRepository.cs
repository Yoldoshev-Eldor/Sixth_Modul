...
public class ToDoItemRepository : IToDoItemRepository
{
    ...
    {
        var toDoItem = await SelectToDoItemByIdAsync(id,userId);
    ...
    public async Task<ICollection<ToDoItem>> SelectAllToDoItemsAsync(long userId, int skip, int take)
    {
        ...
        if (skip < 0 || take <= 0)
        {
            throw new ArgumentOutOfRangeException("Skip must be non-negative and Take must be greater than zero.");
        }
    ...
    {
        var toDoItem = await MainContext.ToDoItems.FirstOrDefaultAsync(x => x.ToDoItemId == id && x.UserId == userId);
    ...
    public async Task UpdateToDoItemAsync(ToDoItem toDoItem, long userId)
    {
        ...
        var existingItem = await MainContext.ToDoItems
            .FirstOrDefaultAsync(item => item.ToDoItemId == toDoItem.ToDoItemId && item.UserId == userId);

        if (existingItem == null)
        {
            throw new UnauthorizedAccessException("You do not have permission to update this item or it does not exist.");
        }
